...
class Component
{
    ...
    public function registerScripts()
    {
        ...
        wp_register_script(
            ...
            $app->publicUrl('js/form-conditionals.js'),
            array('jquery'),
            FLUENTFORM_VERSION,
            true
        );
        ...
        wp_register_style(
            'select2',
            $app->publicUrl('libs/selectWoo/select2.css')
        );

        global $post;
        if (
            (is_a($post, 'WP_Post') && get_post_meta($post->ID, '_has_fluentform', true))  ||
    ...
    public function addFluentFormShortCode()
    {
        ...
        $this->app->addShortCode('fluentform', function ($atts, $content) {
            ...
            $extraCssClass = Helper::getFormInstaceClass($form->id);

            $formBuilder = $this->app->make('formBuilder');
            $output = $formBuilder->build($form, $extraCssClass . ' ff-form-loading');
            ...
            if (did_action('wp_footer') || is_admin()) {
                $form_vars['force_init'] = true;
                ?>
                <script type="text/javascript">
                    if (!window.fluentFormVars) {
                        window.fluentFormVars = <?php echo json_encode($vars); ?>;
                    }
                </script>
                <?php
            } else {
    ...
    private function addInlineVars($vars, $form_id)
    {
        add_action('wp_footer', function () use ($vars, $form_id) {
            ...
            <script type="text/javascript">
                ...
                <?php endif; ?>
                window.fluentFormVars.forms.push(<?php echo $vars; ?>);
            </script>
            <?php
        }, 1000);
    }
